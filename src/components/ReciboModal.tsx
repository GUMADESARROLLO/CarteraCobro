...
export default function ReciboModal({ recibo, onClose }: Props) {
  ...
  const [imagenes, setImagenes] = useState<{ url: string; name: string }[]>([]);
  ...
  useEffect(() => {
    setCargando(true);
    fetch(`/api/imagenes?id_recibo=${encodeURIComponent(recibo.recibo)}`)
      .then(r => r.json())
      .then(data => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="bg-white dark:bg-gray-800 w-full max-w-5xl rounded-xl border border-zinc-200 dark:border-gray-600 shadow-xl overflow-hidden max-h-[90vh] flex flex-col">
        <div className="flex items-start justify-between px-6 py-4 border-b border-zinc-100 dark:border-gray-700 shrink-0">
          <div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div>
              <p className="text-[11px] uppercase tracking-widest text-zinc-400 dark:text-gray-500 mb-2 font-medium">Imagenes del Recibo</p>
              <div className="bg-zinc-50 dark:bg-gray-700/50 rounded-lg border border-zinc-200 dark:border-gray-600 overflow-hidden" style={{ minHeight: 300 }}>
                {cargando ? (
                  <div className="flex items-center justify-center" style={{ height: 300 }}>
                    <div className="w-8 h-8 border-2 border-blue-500 border-t-transparent rounded-full animate-spin"></div>
                  </div>
                ) : imagenes.length === 0 ? (
                  <div className="flex items-center justify-center text-sm text-zinc-400 dark:text-gray-500 italic" style={{ height: 300 }}>
                    Sin imagenes
                  </div>
                ) : (
                  <div className="flex flex-col" style={{ height: 400 }}>
                    <div className="flex-1 bg-black/5 dark:bg-black/20 flex items-center justify-center relative min-h-0">
                      <img
                        src={imagenes[imgIndex].url}
                        alt={imagenes[imgIndex].name}
                      ...
                      {imagenes.length > 1 && (
                        <>
                          <button
                            onClick={(e) => { e.stopPropagation(); setImgIndex(i => (i - 1 + imagenes.length) % imagenes.length); }}
                            className="absolute left-0 inset-y-0 w-12 flex items-center justify-center text-zinc-600 dark:text-gray-300 hover:bg-black/10 dark:hover:bg-white/10 transition-colors z-10"
                          >
                            <svg className="w-6 h-6 drop-shadow-lg" viewBox="0 0 24 24" fill="white" stroke="currentColor" strokeWidth="2"><path d="M15 18l-6-6 6-6" strokeLinecap="round" strokeLinejoin="round"/></svg>
                          </button>
                          <button
                            onClick={(e) => { e.stopPropagation(); setImgIndex(i => (i + 1) % imagenes.length); }}
                            className="absolute right-0 inset-y-0 w-12 flex items-center justify-center text-zinc-600 dark:text-gray-300 hover:bg-black/10 dark:hover:bg-white/10 transition-colors z-10"
                          >
                            <svg className="w-6 h-6 drop-shadow-lg" viewBox="0 0 24 24" fill="white" stroke="currentColor" strokeWidth="2"><path d="M9 18l6-6-6-6" strokeLinecap="round" strokeLinejoin="round"/></svg>
                          </button>
                        </>
                      )}
                    </div>
                    <div className="flex items-center justify-between px-3 py-2 bg-white dark:bg-gray-800 border-t border-zinc-200 dark:border-gray-600">
                      <span className="text-xs text-zinc-500 dark:text-gray-400 truncate">{imagenes[imgIndex].name}</span>
                      <span className="text-xs text-zinc-400 dark:text-gray-500 shrink-0 ml-2">{imgIndex + 1} / {imagenes.length}</span>
                    </div>
                  </div>
                )}
              </div>
        ...
        <div className="px-6 py-4 border-t border-zinc-100 dark:border-gray-700 flex items-center justify-end gap-2 shrink-0">
